...
def infer_mutations(reference_seq, mutant_seq):
    if len(reference_seq) != len(mutant_seq):
        raise ValueError(f"Mutant sequence length does not match reference: reference is length {len(reference_seq)} and mutant_seq is length {len(mutant_seq)}")
    return ":".join(
        f"{ref}{i+1}{mut}" for i, (ref, mut) in enumerate(zip(reference_seq, mutant_seq)) if ref != mut
    )

def repair_mutant_sequence_columns(df, reference_seq, mutant_col='mutant', seq_col='mutated_sequence'):
    df = df.copy()
